...
function finalizeBankAChannels(d, bankA) {
  ...
  for (let j = 0; j < 8; j++) {
    const pad = bankA[j]
    if (pad?.channel != null) {
      // Bytes 116..123: ch pads 1-8; factory captures alias eventType pads 9-16 here.
...
export function patchNameInRaw(raw, name) {
...
export function patchPadInRaw(raw, bank, padIdx, patch) {
  const d = [...raw]
  ...
  if (bank === 'A') {
    ...
    // Pads 9-16 event-type shares 116..123 with ch pads 1-8 — see finalizeBankAChannels.
    if (padIdx < 8) {
...
export function patchKnobInRaw(raw, idx, { cc, ch, min, max }) {
  const d = [...raw]
...
export function patchFaderInRaw(raw, idx, { cc, ch, min, max }) {
  const d = [...raw]
...
export function nameFromRaw(raw) {
...
export function serializeRawFromPreset(preset, orig) {
  let d = [...orig]
